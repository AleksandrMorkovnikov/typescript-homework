...
const howOldWillBeActorAfterTwentyYears = (actor: {age : number}): number => { //Нужно так же добавить типизацию number в этой строчке
  return actor.age + 20;
}
...
document.addEventListener('click', (e) => {
  const coords = [e.clientX, e.clientY]; // Неправильно написано свойство, должно быть clientX/Y
  console.log(`Point is ${coords[0]}, ${coords[1]}`); // Нет бэктиков чтобы вывести clg как строку с включенными в нее данными из функции
});
...
// ошибка - не было закрывающей фигурной скобки, результат не обновляется в аккумуляторе, надо добавить = или return
// функция выводит количество аккаунтов мужчин старше 18 лет из массива
...
function someFunc(data: Human[]): number {
  ...
  return acc + Number(current.age > 18 && current.gender === 'male')}, 0);
};
...
const start = "Сейчас эта надпись будет отзеркалена";
const finish = mirror(start);
console.log(finish);
...
function square(input: number):number {
  ...
  const result = input
    .toString()
    .split('')
    .map(calc => Math.pow(Number(calc), 2))
    .join('')
...
function numberRoot(input: number): number {
  while (input > 10) {
    input = input 
      .toString()
      .split('')
